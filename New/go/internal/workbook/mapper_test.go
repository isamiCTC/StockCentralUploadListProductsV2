...
func TestMapRowsFullImportAcceptsCurrencySymbolInPriceFields(t *testing.T) {
	t.Parallel()
		...
		Rows: []RawRow{
			{
				...
				Values: []string{
					"ABC123", "Producto", "Marca", "Descripcion", "10", "20", "30",
					"1500", "", "$ 1.000", "21", "TIPO", "AHORA", "Cat", "Audio", "5", "$ 900",
				},
			},
		},
	}
	...
	if rows[0].FullImport.ListPrice != 1000 {
		t.Fatalf("ListPrice = %v, want 1000", rows[0].FullImport.ListPrice)
	}
	if rows[0].FullImport.Price != 900 {
		t.Fatalf("Price = %v, want 900", rows[0].FullImport.Price)
	}
}
